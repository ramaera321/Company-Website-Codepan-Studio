        ...
        <div class="col-sm-12">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body add-blog">
                    <form action="/ayat/save" method="post">
                        ...
                        <div class="input-group mb-3">
                            <div class="input-group-prepend">
                                <span class="input-group-text" id="basic-addon1">Judul Ayat</span>
                            </div>
                            <input type="text" class="rounded-right form-control<?= ($validation->hasError('judul_ayat')) ? ' is-invalid' : ''; ?>" placeholder="" aria-label="Username" aria-describedby="basic-addon1" name="judul_ayat" value="<?= old('judul_ayat'); ?>">
                            <div class="invalid-feedback">
                                <?= $validation->getError('judul_ayat'); ?>
                            </div>
                        </div>

                        <div class="form-group">
                            <label for="exampleFormControlTextarea1" class="font-weight-bold">Deskripsi</label>
                            <textarea class="form-control text<?= ($validation->getError('deskripsi')) ? ' is-invalid' : ''; ?>" id="exampleFormControlTextarea1" name="deskripsi" value=""><?= old('deskripsi'); ?></textarea>
                            <div class="invalid-feedback">
                    ...
                    <table class="table table-bordered table-striped table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            foreach ($ayat as $ayat) :
                                $judul_pasal = $pasal->where(['pasal' => $ayat['pasal']])->first();
                            ?>
                                <tr>
                                    <th scope="row"><?= $i; ?></th>
                                    <td><?= $ayat['pasal']; ?></td>
                                    <td><?= $judul_pasal['judul']; ?></td>
                                    <td><?= $ayat['judul_ayat']; ?></td>
                                    <td><?= $ayat['deskripsi']; ?></td>
                                    <td>
                                        <a href="/update_ayat/<?= $ayat['id']; ?>" class="btn bg-warning"></a>
                                        <form action="/ayat/<?= $ayat['id']; ?>" method="post" class="d-inline">
                                            <?= csrf_field(); ?>
